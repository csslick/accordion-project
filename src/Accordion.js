const Accordion = (element, title, options) => {
  ...
  const dropdownHtml = `
    <div class="dropdown relative flex w-auto flex-col border-b border-gray-400">
      <!-- 드롭다운을 열고 닫는 버튼 -->
      <button class="dropdown-toggle flex items-center justify-between rounded-lg bg-linear-to-r from-[#fff] to-[#fff] px-4 py-[10px] font-bold text-black text-xl hover:cursor-pointer hover:opacity-80" type="button" id="dropdownMenuButton" aria-haspopup="true" aria-expanded="false">
        ${title}
